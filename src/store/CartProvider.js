...
const CartProvider = (props) => {
    ...
    const addToCartHandler = async(item) => {
        const existingCartItem = cartData.filter((cartItem) => cartItem.id === item.id)
        if (existingCartItem.length > 0){
            ...
            try{
                ...
                setCartData(updateExistingCartItemAfterPut);
            }catch(error){
                console.log(error)
            }
        }else{
    ...
    const removeFromCartHandler = async(id) => {
        const existingCartItem = cartData.filter((cartItem) => cartItem.id === id)
        if (existingCartItem.length > 0){
